...
void draw() {
  ...
  if(step4)
    {
    for(int i=0; i<10; i++) {M.smoothenInterior(.5);M.smoothenInterior(-.5);} // **04 implement it in Mesh
    M.writeVerticesTo(R);
    }
 ...
 // **05 implement corner operators in Mesh
  ...
  if(step9)
    {
      step10 = false; live = true;
      M.ncars = 0; M.nroads = 0;

      pushMatrix();
      translate(0,0,4); fill(cyan); stroke(dgreen);
      if(live)
        {
        M.reset();
        M.loadVertices(R.G,R.nv);
      ...
      M.showVoronoiEdges();
      M.showArcs();
      //M.drawVoronoiFaceOfInteriorVertices();
      //M.showTriangles();
      noStroke();
      popMatrix();
  ...
  int line=0;
  scribeHeader(" Project 3 for Rossignac's 2018 Graphics Course CS6491 by Wenjie Yao ",line++);
